...
class CalendarService {
  async calculateAndStoreWorkingHours(year: number) {
    ...
    const workingWeekendsRes = await pool.query(
      "SELECT date FROM working_weekend WHERE EXTRACT(YEAR FROM date) = $1",
      [year]
    );
    const workingWeekendDates = workingWeekendsRes.rows.map(
    ...
    for (let month = 0; month < 12; month++) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (
          holidayDates.some((date) => date.getTime() === currentDate.getTime())
        ) {
  ...
  async checkDay(date: string) {
    ...
    const workingWeekendRes = await pool.query(
      "SELECT 1 FROM working_weekend WHERE date = $1",
      [date]
    );
  ...
  async getWorkingHoursInRange(
    ...
    const workingHours = res.rows.map((row) => row.hours);
    const totalHours = workingHours.reduce((sum, hours) => sum + hours, 0);
    workingHours.push(totalHours);
